'''
Собираем статистику по компаниям о возникших проблемах.
'''
...
def process(spark, flights_path, airlines_path, result_path):
    ...
    datamart = flights \
        ...
        .groupBy(airlines['AIRLINE'].alias('AIRLINE_NAME')) \
        .agg(F.count('ARRIVAL_TIME').alias('correct_count'), F.sum('DIVERTED').alias('diverted_count'),
             F.sum('CANCELLED').alias('cancelled_count'), F.avg('DISTANCE').alias('avg_distance'),
             F.avg('AIR_TIME').alias('avg_air_time'),
             F.sum(F.when(flights['CANCELLATION_REASON'] == 'A', 1).otherwise(0)).alias('airline_issue_count'),
             F.sum(F.when(flights['CANCELLATION_REASON'] == 'B', 1).otherwise(0)).alias('weather_issue_count'),
             F.sum(F.when(flights['CANCELLATION_REASON'] == 'C', 1).otherwise(0)).alias('nas_issue_count'),
             F.sum(F.when(flights['CANCELLATION_REASON'] == 'D', 1).otherwise(0)).alias('security_issue_count')
             ) \
        .select(F.col('AIRLINE_NAME'),
                F.col('correct_count'), F.col('diverted_count'), F.col('cancelled_count'), F.col('avg_distance'),
                F.col('avg_air_time'), F.col('airline_issue_count'), F.col('weather_issue_count'),
